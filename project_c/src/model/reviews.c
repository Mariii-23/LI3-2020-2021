...
void set_review_date(Review self, char* date) {
    if (self && date) self->date = g_strdup(date);
}
...
int get_number_reviews_by_business(ReviewCollection self, char* business_id) {
    return get_reviewCollection_reviews_by_business_id(self, business_id)->len;
}

void set_reviewCollection_by_user(ReviewCollection self, GHashTable* by_user) {
    if (self && by_user) self->by_user_id = by_user;
}

GPtrArray* get_reviewCollection_reviews_by_business_id(
...
void set_reviewCollection_by_business(
    ...
    if (self && by_business) self->by_business_id = by_business;
}
