...
void free_g_city_tuple(gpointer data) {
    free_city_tuple((CityTuple) data);
}
...
void add_city_to_business_by_star(
    Stats stats, char* city, char* business_id, float stars, char* name) {
    if (!stats || !stats->business_id_to_stars) return;
    ...
    if (!aux) {
        ...
        g_hash_table_insert(stats->city_to_business_by_star, city, list);
        return;
    }

    GSList* new_list =
        g_slist_insert_sorted_with_data(aux, value, compare_stars, NULL);
    g_hash_table_insert(stats->city_to_business_by_star, city, new_list);
}

void add_category_to_business_by_star(
    Stats stats, char* category, char* business_id, float stars, char* name) {
    if (!stats || !stats->business_id_to_stars) return;
    ...
    if (!aux) {
        ...
        list = g_slist_append(list, value);
        g_hash_table_insert(
            stats->category_to_business_by_star, category, list);
        return;
    }
    ...
    GSList* new_list =
        g_slist_insert_sorted_with_data(aux, value, compare_stars, NULL);
    g_hash_table_insert(
        stats->category_to_business_by_star, category, new_list);
}
...
static GSList* n_larger_gs_list(int N, GSList* gs_list) {
    ...
    for (int i = 0; i < size && i < N; i++)
        new =
            g_slist_append(new, copy_city_tuple(g_slist_nth_data(gs_list, i)));
...
void n_larger_city_star(
    Stats stats, char* city, int N, TABLE table, int larger_than) {
    if (!stats && !stats->city_to_business_by_star) return;
    ...
    GSList* list =
        larger_than
            ...
            : n_larger_gs_list(
                  N,
                  g_hash_table_lookup(stats->city_to_business_by_star, city));
    ...
    while ((value = g_slist_nth_data(list, i)) != NULL) {
        ...
        if (larger_than == 0) add_field(table, stars);
        free(id);
        free(stars);
        i++;
    }
