...
void add_business(BusinessCollection self, Business elem) {
  if (self && elem)
    g_ptr_array_add(self->businesses, elem);
}
...
Business get_businessCollection_business_by_name(BusinessCollection self,
                                                 char *name) {
  if (self)
...
void add_businessCollection_by_name(BusinessCollection self,
                                    Business business) {
  if (self)
...
void free_businessCollection(BusinessCollection self) {
  if (self) {
    ...
    g_tree_foreach(self->by_name, free_key_tree, NULL);
