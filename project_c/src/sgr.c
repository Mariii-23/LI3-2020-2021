...
#define QUERY_FIVE_FIELDS 3
#define QUERY_SIX_FIELDS 3
#define QUERY_SEVEN_FIELDS 1
#define QUERY_EIGHT_FIELDS 3
...
TABLE businesses_started_by_letter(SGR sgr, char letter) {
  ...
  char *fields[] = {"Name"};
  table->field_names = build_head(fields, QUERY_TWO_FIELDS);
...
TABLE business_info(SGR sgr, char *business_id) {
  ...
  table->field_names = build_head(fields, QUERY_THREE_FIELDS);
...
TABLE bussinesses_reviewed(SGR sgr, char *id) {
  ...
  table->field_names = build_head(fields, QUERY_FOUR_FIELDS);
...
TABLE businesses_with_stars_and_city(SGR sgr, float stars, char *city) {
  ...
  table->field_names = build_head(fields, QUERY_FIVE_FIELDS);
...
TABLE top_businesses_by_city(SGR sgr, int top) {
  TABLE table = malloc(sizeof(struct table));
  char *fields[] = {"id", "name", "stars"};
  table->field_names = build_head(fields, QUERY_SIX_FIELDS);
